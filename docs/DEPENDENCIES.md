...
| Install (Arch) | AUR `whisper.cpp` — `paru -S whisper.cpp` (or `yay -S whisper.cpp`) installs `whisper-cli`; or build once locally — `cmake`/`make` per the project's own instructions. Model `.bin` fetched once via `clipper fetch-model` (HuggingFace, sha256 recorded) into `model/`. Burn needs a usable font: Arch's default fontconfig ships DejaVu Sans — the ASS default (`captions.ass_style.font`) — no extra package. |
